...
defineRouteMeta({
  openAPI: {
    ...
    summary: 'Delete a translation for a course',
    parameters: [
...
export default defineEventHandler(async (event) => {
  ...
  const [deleted] = await db.delete(courseTranslations)
    .where(and(eq(courseTranslations.courseId, entityId), eq(courseTranslations.locale, locale)))
    .returning()
  if (!deleted) {
